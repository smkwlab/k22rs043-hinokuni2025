...
Department of Information Science, Faculty of Science and Technology, Kyushu Sangyo University, Fukuoka 813--8503, Japan}
...
我々の研究室ではこれを支援するシステム「サポちゃん」\cite{bib:densi2016,bib:hinokuni2022}を開発してきたが，管理者向け機能が未整備で，データベースを直接操作する必要があり，運用負担やヒューマンエラーのリスクが課題であった．
...
\begin{ekeyword}
Exercise Support System, Administrator Functions, Programming Education
\end{ekeyword}
%
%\begin{jkeyword}
...
我々の研究室では，このような授業を支援することを目的として，演習支援システム「サポちゃん」を開発している．
サポちゃんは，質問や演習チェックの依頼をボタン操作で行える機能を備え，授業運営の効率化に寄与している．

一方で，これまでの開発は受講生およびSA/TA・教員向け機能が中心であり，管理者向け機能が十分に整備されていなかった．
そのため，授業情報の登録にはデータベースの直接操作が必要となり，専門的知識を要するうえ，作業負担やヒューマンエラーのリスクが高かった．
その他にもSA/TA・教員用画面の機能不足，クォーター制への未対応，授業情報の一括登録不可など，運用上の課題が多く存在した．

そこで本研究では，サポちゃんにおける管理者の運用負担の軽減を目的とし，管理者機能を開発した．
また，その他の運用上の課題を解決するために，機能の改良および新規追加を行った．
...
\begin{description}
  ...
  \item[ユーザー編集機能]
    ...
    ユーザーID，パスワード，ユーザー名，ユーザー名（カナ），ユーザータイプ，管理者権限の有無を更新できる．
    LDAP認証ユーザーについては，パスワード欄が空欄の状態でも更新が可能である．
  \item[ユーザー削除機能]
    指定したユーザーをシステムから削除する機能である．
  \item[ユーザー一覧表示機能]
    各ユーザーのユーザーID，ユーザー名，ユーザー名（カナ），ユーザータイプ，管理者権限の有無を表示する機能である．
    20名ごとにページネーションで切り替えて表示する．
    ユーザー管理画面を図\ref{fig:yu-za-kannri}に示す．
    \begin{figure}[H]
      \centering
      \includegraphics[clip, width=\columnwidth, keepaspectratio]{img/yu-za-kannri.png}
      \caption{ユーザー管理画面}
      \ecaption{User Management Page}
      \label{fig:yu-za-kannri}
    \end{figure}
  \item[ユーザー検索機能]
  ...
  \item[受講生一覧表示機能]
    ...
    ユーザーID，ユーザー名，ユーザー名（カナ）を表示し，20名ごとにページネーションで切り替えて表示する．
    受講生編集画面を図\ref{fig:zyukouseihennsyuu}に示す．
    \begin{figure}[H]
      \centering
      \includegraphics[clip, width=\columnwidth, keepaspectratio]{img/zyukouseihennsyuu.png}
      \caption{受講生編集画面}
      \ecaption{Student Edit Page}
      \label{fig:zyukouseihennsyuu}
    \end{figure}
  \item[受講生解除機能]
...
\begin{description}
  ...
  \item[科目一覧表示機能]
    ...
    科目管理画面を図\ref{fig:kamokukannri}に示す．
    \begin{figure}[H]
      \centering
      \includegraphics[clip, width=\columnwidth, keepaspectratio]{img/kamokukannri.png}
      \caption{科目管理画面}
      \ecaption{Subject Management Page}
      \label{fig:kamokukannri}
    \end{figure}
  \item[科目編集機能]
...
\begin{description}
  ...
  \item[授業一覧表示機能]
    ...
    授業・演習管理画面を図\ref{fig:zyugyoukannri}に示す．
      \begin{figure}[H]
      \centering
      \includegraphics[clip, width=\columnwidth, keepaspectratio]{img/zyugyoukannri.png}
      \caption{授業・演習管理画面}
      \ecaption{Class and Exercise Management Page}
      \label{fig:zyugyoukannri}
    \end{figure}
  \item[授業編集機能]
    登録されている授業の詳細情報を更新する．
  \item[授業削除機能]
...
\begin{description}
  \item[チェックコメント一覧表示機能]
    ...
    演習チェック画面を図\ref{fig:ensyuucheck}に示す．
    \begin{figure}[H]
      \centering
      \includegraphics[clip, width=\columnwidth, keepaspectratio]{img/ensyuucheck.png}
      \caption{演習チェック画面}
      \ecaption{Exercise Check Page}
      \label{fig:ensyuucheck}
    \end{figure}
...
\section{サポちゃん2025の実装}

サポちゃん2025の実装環境を表\ref{tb:zissoukankyou}に示す．

\begin{table}[H]
  \centering
  \caption{実装環境}
  \ecaption{Implementation Environment}
  \begin{tabular}{|l|l|l|}\hline
    \multicolumn{1}{|c|}{用途} & \multicolumn{1}{c|}{名称} & \multicolumn{1}{c|}{バージョン}\\ \hline\hline
    仮想化ソフトウェア &  Docker & 27.3.1  \\ \hline
    Webサーバ & Apache & 2.4.62 \\ \hline
    使用言語 & PHP & 8.3.22  \\ \hline
    データベース & MySQL & 8.0.41 \\ \hline
    Webレイアウト & Bootstrap & 4.3.1  \\ \hline
  \end{tabular}
  \label{tb:zissoukankyou}
\end{table}
...
\begin{table}[H]
  \centering
  \caption{評価内容と対象者}
  \ecaption{Evaluation Content and Target Users}
...
\subsection{評価結果}\label{sec:hyoukaresult}
以下に各アンケートの評価結果を示す．
自由記述の回答は一部を要約して掲載する．
なお，括弧内の数字は，各回答者が選択した選択肢を示している．
また，一部の回答の明らかな誤字・脱字は修正して掲載している．

\subsubsection{管理者機能の評価結果}
3名から回答があった．
...
\begin{table}[H]
  \centering
  \caption{管理者機能の評価結果}
  \ecaption{Evaluation Results of Administrator Functions}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item SA/TAを登録する際に，ユーザーIDユーザー名で検索でき，視覚的にわかりやすくて楽（5）
    \end{itemize}
...
\subsubsection{SA/TA・教員用画面の改善内容の評価結果}
8名から回答があった．
...
\begin{table}[H]
  ...
  \ecaption{Evaluation Results of SA/TA and Instructor Screen Improvements}
  \begin{tabular}{|p{7cm}|c|}\hline
    \multicolumn{1}{|c|}{アンケート項目} & \multicolumn{1}{c|}{平均}  \\ \hline\hline
...
\subsubsection{システム全体の評価結果}
3名から回答があった．
システム全体の評価アンケートの結果を表\ref{tb:zentai}に示す．
\begin{table}[H]
  \centering
  \caption{システム全体の評価結果}
  \ecaption{Evaluation Results of the Entire System}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        また，手を挙げて質問しづらい子の質問にも気づきやすくなるので，学生側の評判も確認しながら細かい部分で機能が改善されるようであれば今後の授業でも使ってみたい．（4）
        \item 便利だから．（5）
    \end{itemize}
...
\section{おわりに}
本研究では既存のサポちゃんの課題を解決するため「サポちゃん2025」を開発し，管理者向け Web UI の実装，CSV・Excel による授業情報の一括登録，クォーター制への対応，SA/TA・教員用画面の機能改善を行った．評価実験の結果，各利用者から高い評価を得ており，管理者および利用者の負担軽減と利便性向上が確認された．

一方，今後の課題として管理者機能のスマートフォン対応と管理者権限の細分化が挙げられる．
スマートフォン向け UI を開発することで管理者の利便性向上が期待される．
また，管理者権限を細分化し，教員が担当科目のみを操作可能とすることで，誤操作防止や操作性の向上につながると考える．



\begin{thebibliography}{10}

\bibitem{bib:densi2016} 
中島礼乃，古井陽之助，神屋郁子，下川俊彦：講義における演習の進捗状況把握支援システムの開発，電子情報通信学会2016年総合大会（2016）.

\bibitem{bib:hinokuni2022}
本村涼， 神屋郁子，下川俊彦：ハイブリッドな授業形態で利用可能な演習支援システムの開発，火の国シンポジウム2022（2022）.
